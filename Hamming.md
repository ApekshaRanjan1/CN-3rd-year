...
Hamming code is a popular error detection and error correction method in data communication. Hamming code can only detect 2-bit errors and correct a single bit error, which means it is unable to correct burst errors that may occur during data transmission.

## Redundant Bits Calculation

Hamming code uses redundant bits (extra bits) which are calculated according to the following formula:

```
2^r ≥ m + r + 1
```
Where:
- `r` is the number of redundant bits required.
- `m` is the number of data bits.

`r` is calculated by increasing the value from 1, 2, 3, etc., until the equation becomes true.
...
These redundant bits are then added to the original data for the calculation of errors at the receiver's end.

## Error Correction at the Receiver's End

At the receiver's end, with the help of even parity (commonly used), the erroneous bit position is identified. Since the data is in binary, we take the complement of the erroneous bit position to correct the received data.

The respective index parity is calculated for R₁, R₂, R₃, R₄, and so on.
![image](https://github.com/user-attachments/assets/91cebcc3-4b83-4dba-9311-ed778b8eabe5)
